...
async function getHistoryExchange(symbol,resolution, from, to, countback) {
  ...
  const config2 = {"s":"no_data","nextTime":1522108800};  
    ...
    const CR = currency[1].toUpperCase();
      ...
      console.log("error in : fucntion.chart.js > getHistoryExchange.js : ",error.message)
      return "error in : fucntion.chart.js > getHistoryExchange.js : "+error.message;
  }
    // }else{
    //   return config2;
    // }
...
//     let remoteTrade = false;
//     const walletsymbol = currency[0].toUpperCase();
//     let find_remote = await remote_trade.findOne({currency_type:walletsymbol,status:true})
//     if(find_remote){
//       sy = find_remote.graph;
//       price= find_remote.price;
//       remoteTrade = true;
...
//       const data    = await getChartDataByCurrency(currency_type, compare_currency);
//       const newS    = symbol.replace('-','').toUpperCase();
//       return data.ohlc[newS]
//     }
...
//     // }
// }
function arrangeData(result,price,avg,high,low){
  try{
    let ct  = 86400;
    let limit = 1000;
    if( result.length > 0) {
...
async function gettimeScaleExchange(symbol,resolution, from, to, countback) {
  try{  
    ...
    // // window.localStorage.setItem(symbol, 0);
    // // if(window.localStorage.getItem(symbol) == 1){
    //   const sy =  symbol ? symbol.replace("-", "").toLowerCase() : symbol;
    //   const rp = require('request-promise');
    //   const requestOptions = {
    //       method: 'GET',
    //       uri: `https://x.wazirx.com/api/v2/k`,
...
async function storeOHLCCustom(currency_type) {
  const remote_trade = require("../models/remote_trading");
  let find_remote = await remote_trade.findOne({currency_type:currency_type,status:true})
  let gChat = {};
  if(find_remote){
      let fs = require('fs');
      path = require('path')
      dirname = path.join(__dirname, `../json/ohlc_${currency_type}.json`);
      gChat = await getHistoryExchangeDummy(currency_type, find_remote.price, find_remote.update_price,find_remote.inrgraph,find_remote.usdtgraph,find_remote.btcgraph,find_remote.vrxgraph)
      let jsonD = JSON.stringify(gChat);
      fs.writeFile(dirname, jsonD, 'utf8', (d) => {
          console.log(d);
      });
  }
  return gChat;
