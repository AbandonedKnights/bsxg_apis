...
function validateAmount(amount) {
    ...
    // console.log("t>>>>>",typeof(amount), amount)
    if (amount <= 0) {
        return false;
    }
...
function validatePrice(price) {
    ...
    console.log("t>>>>>", typeof (price), price)
    if (price <= 0) {
        return false;
    }
...
function totalwithdrawFees(wallets,index,currency){
    let newAR = {};
    let pushVal = [];
    currency = currency ? currency : wallet_type;
    wallets.map((i2) =>{
        let wt = (currency == 'total') ? 'total' : i2[currency] ? i2[currency].toUpperCase() : '';
        let bl = (index == 'count') ? 1 : i2[index];
        if(pushVal.includes(wt)){
            let nl = parseFloat(newAR[wt]) + parseFloat(bl);
            newAR[wt] = isFloat(nl) ? nl.toFixed(2) : parseInt(nl); 
        }else{
            pushVal.push(wt)
            newAR[wt] = isFloat(bl) ? parseFloat(bl) : parseInt(bl);
        }
    })
    return newAR;
}
function totalTradeCommission(wallets,index){
    let newAR = {};
    let pushVal = [];
    wallets.map((i2) =>{
        let wt = i2.currency_type.toUpperCase();
        let tl = i2[index] && i2[index].indexOf("+") >= 0 ? i2[index].split('+') : [];
        let bl = tl.length > 1 ? parseFloat(tl[0])+parseFloat(tl[1]) : 0;  
        if(pushVal.includes(wt)){
            let nl = parseFloat(newAR[wt]) + parseFloat(bl);
